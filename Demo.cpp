...
int main(void)
{
    ...
    printf("-- Delegate demo --\n");
    printf("A no-parameter delegate is declared using Delegate<void()>\n\n");

    Delegate<void()> del1(&SimpleVoidFunction);
    del1();

    Delegate<void()> del2([]() { printf("In simple lambda with no parameters.\n"); });
    del2();

    auto lambda2 = []() { printf("In simple lambda2\n"); };
    del1 = lambda2;
    del2 = lambda2;
    printf("Same delegates are equal? %s\n", (del1 == del2) ? "yes" : "NO!");
    del1();
    del2();

    del2 = &SimpleVoidFunction;
    printf("Different delegates are equal? %s\n", (del1 == del2) ? "YES!" : "no");
    del2();

    printf("\n");
    printf("-- Examples using two-parameter delegates (int, const char*) --\n\n");

    typedef Delegate<void(int, const char*)> MyDelegate;
    typedef Delegate<int(int, const char*)> IntMyDelegate;
